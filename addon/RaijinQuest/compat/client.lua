...
RaijinQuestCompat = {}
RaijinQuestCompat.mod = mod or math.mod
RaijinQuestCompat.gfind = string.gmatch or string.gfind
RaijinQuestCompat.itemsuffix = client > 11200 and ":0:0:0:0:0:0:0" or ":0:0:0"
RaijinQuestCompat.rotateMinimap = client > 11200 and GetCVar("rotateMinimap") ~= "0" and true or nil
RaijinQuestCompat.client = client
...
RaijinQuestCompat.GetQuestLogTitle = function(id)
...
RaijinQuestCompat.GetDifficultyColor = GetQuestDifficultyColor or GetDifficultyColor

-- wotlk: changed from QuestWatchFrame to WatchFrame in 3.3
RaijinQuestCompat.QuestWatchFrame = QuestWatchFrame or WatchFrame

-- wotlk: changed questlog related frame names in 3.3
RaijinQuestCompat.QuestLogQuestTitle = QuestLogQuestTitle or QuestInfoTitleHeader
RaijinQuestCompat.QuestLogObjectivesText = QuestLogObjectivesText or QuestInfoObjectivesText
RaijinQuestCompat.QuestLogQuestDescription = QuestLogQuestDescription or QuestInfoDescriptionText
RaijinQuestCompat.QuestLogDescriptionTitle = QuestLogDescriptionTitle or QuestInfoDescriptionHeader

-- vanilla+tbc+wotlk: base function to insert quest links to the chat
RaijinQuestCompat.InsertQuestLink = function(questid, name)
  local questid = questid or 0
  local fallback = name or UNKNOWN
  local level = RaijinQuestDB["quests"]["data"][questid] and RaijinQuestDB["quests"]["data"][questid]["lvl"] or 0
  local name = RaijinQuestDB["quests"]["loc"][questid] and RaijinQuestDB["quests"]["loc"][questid]["T"] or fallback
  local hex = pfUI.api.rgbhex(RaijinQuestCompat.GetDifficultyColor(level))

  ChatFrameEditBox:Show()
  if RaijinQuest_config["questlinks"] == "1" then
    ...
    if text then
      for oldhex, questid, level in gfind(text, "(|c%x+)|Hquest:(.-):(.-)|h") do
        ...
        if not level or level == 0 then
          level = RaijinQuestDB["quests"]["data"][questid] and RaijinQuestDB["quests"]["data"][questid]["lvl"] or 0
        end

        if level and level > 0 then
          local newhex = pfUI.api.rgbhex(RaijinQuestCompat.GetDifficultyColor(level))
          text = string.gsub(text, oldhex .. "|Hquest:"..questid, newhex.."|Hquest:"..questid)
        end
      end
    end

    frame.RaijinQuestHookAddMessage(frame, text, a1, a2, a3, a4, a5)
  end

  for i=1,NUM_CHAT_WINDOWS do
    _G["ChatFrame"..i].RaijinQuestHookAddMessage = _G["ChatFrame"..i].RaijinQuestHookAddMessage or _G["ChatFrame"..i].AddMessage
