...
for _, exp in pairs({ "-tbc", "-wotlk" }) do
  for _, db in pairs(dbs) do
    if RaijinQuestDB[db]["data"..exp] then
      patchtable(RaijinQuestDB[db]["data"], RaijinQuestDB[db]["data"..exp])
    end

    for loc, _ in pairs(RaijinQuestDB.locales) do
      if RaijinQuestDB[db][loc] and RaijinQuestDB[db][loc..exp] then
        loc_update = RaijinQuestDB[db][loc..exp] or RaijinQuestDB[db]["enUS"..exp]
  ...
  loc_update = RaijinQuestDB["professions"][loc..exp] or RaijinQuestDB["professions"]["enUS"..exp]
  if loc_update then patchtable(loc_core, loc_update) end

  if RaijinQuestDB["minimap"..exp] then patchtable(RaijinQuestDB["minimap"], RaijinQuestDB["minimap"..exp]) end
  if RaijinQuestDB["meta"..exp] then patchtable(RaijinQuestDB["meta"], RaijinQuestDB["meta"..exp]) end
end

-- detect installed locales
for key, name in pairs(RaijinQuestDB.locales) do
  if not RaijinQuestDB["quests"][key] then RaijinQuestDB.locales[key] = nil end
end

-- detect localized databases
RaijinQuestDatabase.dbstring = ""
for id, db in pairs(dbs) do
  -- assign existing locale
  RaijinQuestDB[db]["loc"] = RaijinQuestDB[db][loc] or RaijinQuestDB[db]["enUS"] or {}
  RaijinQuestDatabase.dbstring = RaijinQuestDatabase.dbstring .. " |cffcccccc[|cffffffff" .. db .. "|cffcccccc:|cff33ffcc" .. ( RaijinQuestDB[db][loc] and loc or "enUS" ) .. "|cffcccccc]"
end

-- track questitems to maintain object requirements
RaijinQuestDatabase.itemlist = CreateFrame("Frame", "RaijinQuestDatabaseQuestItemTracker", UIParent)
RaijinQuestDatabase.itemlist.update = 0
RaijinQuestDatabase.itemlist.db = {}
RaijinQuestDatabase.itemlist.db_tmp = {}
RaijinQuestDatabase.itemlist.registry = {}
RaijinQuestDatabase.TrackQuestItemDependency = function(self, item, qid)
  self.itemlist.registry[item] = qid
  self.itemlist.update = GetTime() + .5
  self.itemlist:Show()
end

RaijinQuestDatabase.itemlist:RegisterEvent("BAG_UPDATE")
RaijinQuestDatabase.itemlist:SetScript("OnEvent", function()
  this.update = GetTime() + .5
  this:Show()
end)

RaijinQuestDatabase.itemlist:SetScript("OnUpdate", function()
  if GetTime() < this.update then return end

  -- remove obsolete registry entries
  for item, qid in pairs(this.registry) do
    if not RaijinQuest.questlog[qid] then
  ...
  for item in pairs(this.db) do
    if not previous[item] and this.registry[item] then
      RaijinQuest.questlog[this.registry[item]] = nil
...
CreateFrame("Frame", "RaijinQuestLocaleCheck", UIParent):SetScript("OnUpdate", function()
  ...
  if ItemRefTooltip:IsShown() and ItemRefTooltipTextLeft1 and ItemRefTooltipTextLeft1:IsVisible() then
    ...
    -- check for noloc
    if name and name ~= "" and RaijinQuestDB["items"][loc] and RaijinQuestDB["items"][loc][6948] then
      if not strfind(name, RaijinQuestDB["items"][loc][6948], 1) then
        RaijinQuestDatabase.dbstring = ""
        for id, db in pairs(dbs) do
          -- assign existing locale and update dbstring
          RaijinQuestDB[db]["loc"] = noloc[db] and RaijinQuestDB[db]["enUS"] or RaijinQuestDB[db][loc] or {}
          RaijinQuestDatabase.dbstring = RaijinQuestDatabase.dbstring .. " |cffcccccc[|cffffffff" .. db .. "|cffcccccc:|cff33ffcc" .. ( noloc[db] and "enUS" or loc ) .. "|cffcccccc]"
        end
      end

      RaijinQuestDatabase.localized = true
      this:Hide()
    end
  end
...
if isempty(RaijinQuestDB["quests"]["loc"]) then
  CreateFrame("Frame"):SetScript("OnUpdate", function()
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cffffccccFor more details, see: https://shagu.org/RaijinQuest")
    this:Hide()
  end)
end
...
RaijinQuestDatabase.Reload = function()
  items = RaijinQuestDB["items"]["data"]
  units = RaijinQuestDB["units"]["data"]
  objects = RaijinQuestDB["objects"]["data"]
  quests = RaijinQuestDB["quests"]["data"]
  zones = RaijinQuestDB["zones"]["data"]
  refloot = RaijinQuestDB["refloot"]["data"]
  itemreq = RaijinQuestDB["quests-itemreq"]["data"]
  areatrigger = RaijinQuestDB["areatrigger"]["data"]
  professions = RaijinQuestDB["professions"]["loc"]
end
...
if RaijinQuestCompat.client > 11200 then
  bitraces[512] = "BloodElf"
  bitraces[1024] = "Draenei"
end

-- make it public for extensions
RaijinQuestDB.bitraces = bitraces

local bitclasses = {
  [Enum.ClassMask.WARRIOR] = "WARRIOR",
  [Enum.ClassMask.PALADIN] = "PALADIN",
  [Enum.ClassMask.HUNTER] = "HUNTER",
  [Enum.ClassMask.ROGUE] = "ROGUE",
  [Enum.ClassMask.PRIEST] = "PRIEST",
  [Enum.ClassMask.DEATHKNIGHT] = "DEATHKNIGHT",
  [Enum.ClassMask.SHAMAN] = "SHAMAN",
  [Enum.ClassMask.MAGE] = "MAGE",
  [Enum.ClassMask.WARLOCK] = "WARLOCK",
  [Enum.ClassMask.HERO] = "HERO",
  [Enum.ClassMask.DRUID] = "DRUID",
  [Enum.ClassMask.BARBARIAN] = "BARBARIAN",
  [Enum.ClassMask.WITCHDOCTOR] = "WITCHDOCTOR",
  [Enum.ClassMask.DEMONHUNTER] = "DEMONHUNTER",
  [Enum.ClassMask.WITCHHUNTER] = "WITCHHUNTER",
  [Enum.ClassMask.STORMBRINGER] = "STORMBRINGER",
  [Enum.ClassMask.FLESHWARDEN] = "FLESHWARDEN",
  [Enum.ClassMask.GUARDIAN] = "GUARDIAN",
  [Enum.ClassMask.MONK] = "MONK",
  [Enum.ClassMask.SONOFARUGAL] = "SONOFARUGAL",
  [Enum.ClassMask.RANGER] = "RANGER",
  [Enum.ClassMask.CHRONOMANCER] = "CHRONOMANCER",
  [Enum.ClassMask.NECROMANCER] = "NECROMANCER",
  [Enum.ClassMask.PYROMANCER] = "PYROMANCER",
  [Enum.ClassMask.CULTIST] = "CULTIST",
  [Enum.ClassMask.STARCALLER] = "STARCALLER",
  [Enum.ClassMask.SUNCLERIC] = "SUNCLERIC",
  [Enum.ClassMask.TINKER] = "TINKER",
  [Enum.ClassMask.PROPHET] = "PROPHET",
  [Enum.ClassMask.REAPER] = "REAPER",
  [Enum.ClassMask.WILDWALKER] = "WILDWALKER",
  [Enum.ClassMask.SPIRITMAGE] = "SPIRITMAGE",
}

-- make it public for extensions
RaijinQuestDB.bitclasses = bitclasses

function RaijinQuestDatabase:IsFriendly(id)
...
function RaijinQuestDatabase:BuildQuestDescription(meta)
  if not meta.title or not meta.quest or not meta.QTYPE then return meta.description end

  if meta.QTYPE == "NPC_START" then
    return string.format(RaijinQuest_Loc["Speak with |cff33ffcc%s|r to obtain |cffffcc00[!]|cff33ffcc %s|r"], (meta.spawn or UNKNOWN), (meta.quest or UNKNOWN))
  elseif meta.QTYPE == "OBJECT_START" then
    return string.format(RaijinQuest_Loc["Interact with |cff33ffcc%s|r to obtain |cffffcc00[!]|cff33ffcc %s|r"], (meta.spawn or UNKNOWN), (meta.quest or UNKNOWN))
  elseif meta.QTYPE == "NPC_END" then
    return string.format(RaijinQuest_Loc["Speak with |cff33ffcc%s|r to complete |cffffcc00[?]|cff33ffcc %s|r"], (meta.spawn or UNKNOWN), (meta.quest or UNKNOWN))
  elseif meta.QTYPE == "OBJECT_END" then
    return string.format(RaijinQuest_Loc["Interact with |cff33ffcc%s|r to complete |cffffcc00[?]|cff33ffcc %s|r"], (meta.spawn or UNKNOWN), (meta.quest or UNKNOWN))
  elseif meta.QTYPE == "UNIT_OBJECTIVE" then
    if RaijinQuestDatabase:IsFriendly(meta.spawnid) then
      return string.format(RaijinQuest_Loc["Talk to |cff33ffcc%s|r"], (meta.spawn or UNKNOWN))
    else
      return string.format(RaijinQuest_Loc["Kill |cff33ffcc%s|r"], (meta.spawn or UNKNOWN))
    end
  elseif meta.QTYPE == "UNIT_OBJECTIVE_ITEMREQ" then
    return string.format(RaijinQuest_Loc["Use |cff33ffcc%s|r on |cff33ffcc%s|r"], (meta.itemreq or UNKNOWN), (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "OBJECT_OBJECTIVE" then
    return string.format(RaijinQuest_Loc["Interact with |cff33ffcc%s|r"], (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "OBJECT_OBJECTIVE_ITEMREQ" then
    return string.format(RaijinQuest_Loc["Use |cff33ffcc%s|r at |cff33ffcc%s|r"], (meta.itemreq or UNKNOWN), (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "ITEM_OBJECTIVE_LOOT" then
    return string.format(RaijinQuest_Loc["Loot |cff33ffcc[%s]|r from |cff33ffcc%s|r"], (meta.item or UNKNOWN), (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "ITEM_OBJECTIVE_USE" then
    return string.format(RaijinQuest_Loc["Loot and/or Use |cff33ffcc[%s]|r from |cff33ffcc%s|r"], (meta.item or UNKNOWN), (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "AREATRIGGER_OBJECTIVE" then
    return string.format(RaijinQuest_Loc["Explore |cff33ffcc%s|r"], (meta.spawn or UNKNOWN))
  elseif meta.QTYPE == "ZONE_OBJECTIVE" then
    return string.format(RaijinQuest_Loc["Use Quest Item at |cff33ffcc%s|r"], (meta.spawn or UNKNOWN))
  end
end

-- ShowExtendedTooltip
-- Draws quest informations into a tooltip
function RaijinQuestDatabase:ShowExtendedTooltip(id, tooltip, parent, anchor, offx, offy)
  ...
  local locales = RaijinQuestDB["quests"]["loc"][id]
  local data = RaijinQuestDB["quests"]["data"][id]
  ...
  if data then
    ...
    if queststate == 0 then
      tooltip:AddLine(RaijinQuest_Loc["You don't have this quest."] .. "\n\n", 1, .5, .5)
    elseif queststate == 1 then
      tooltip:AddLine(RaijinQuest_Loc["You are on this quest."] .. "\n\n", 1, 1, .5)
    elseif queststate == 2 then
      tooltip:AddLine(RaijinQuest_Loc["You already did this quest."] .. "\n\n", .5, 1, .5)
    end
      ...
      for key, db in pairs({["U"]="units", ["O"]="objects", ["I"]="items"}) do
        if data["start"][key] then
          ...
          for _, id in pairs(data["start"][key]) do
            entries = entries .. (entries == "" and "" or ", ") .. ( RaijinQuestDB[db]["loc"][id] or UNKNOWN )
          end

          tooltip:AddDoubleLine(RaijinQuest_Loc["Quest Start"]..":", entries, 1,1,1, 1,1,.8)
        end
      end
    end
    ...
    if data["end"] then
      for key, db in pairs({["U"]="units", ["O"]="objects"}) do
        if data["end"][key] then
          ...
          tooltip:AddDoubleLine(RaijinQuest_Loc["Quest End"]..":", entries, 1,1,1, 1,1,.8)
        end
      end
    end
  end
  ...
  if locales then
    ...
    if locales["O"] and locales["O"] ~= "" then
      tooltip:AddLine(" ")
      tooltip:AddLine(RaijinQuestDatabase:FormatQuestText(locales["O"]),1,1,1,true)
    end

    -- details
    if locales["D"] and locales["D"] ~= "" then
      tooltip:AddLine(" ")
      tooltip:AddLine(RaijinQuestDatabase:FormatQuestText(locales["D"]),.6,.6,.6,true)
    end
  end
  ...
  if data then
    ...
    if data["lvl"] then
      ...
      tooltip:AddLine("|cffffffff" .. RaijinQuest_Loc["Quest Level"] .. ": |r" .. questlevel, color.r, color.g, color.b)
    end
    if data["min"] then
      local questlevel = tonumber(data["min"])
      local color = RaijinQuestCompat.GetDifficultyColor(questlevel)
      tooltip:AddLine("|cffffffff" .. RaijinQuest_Loc["Required Level"] .. ": |r" .. questlevel, color.r, color.g, color.b)
    end
  end
...
function RaijinQuestDatabase:GetPlayerSkill(skill)
...
function RaijinQuestDatabase:GetBitByRace(model)
...
function RaijinQuestDatabase:GetBitByClass(class)
...
function RaijinQuestDatabase:GetHexDifficultyColor(level, force)
  if force and UnitLevel("player") < level then
    return "|cffff5555"
  else
    local c = RaijinQuestCompat.GetDifficultyColor(level)
    return string.format("|cff%02x%02x%02x", c.r*255, c.g*255, c.b*255)
  end
end

-- GetRaceMaskByID
function RaijinQuestDatabase:GetRaceMaskByID(id, db)
  ...
  elseif RaijinQuestDB[db] and RaijinQuestDB[db]["data"] and RaijinQuestDB[db]["data"]["fac"] then
    raceMask = factionMap[RaijinQuestDB[db]["data"]["fac"]]
  end
...
function RaijinQuestDatabase:GetIDByName(name, db, partial, server)
  if not RaijinQuestDB[db] then return nil end
  local ret = {}

  for id, loc in pairs(RaijinQuestDB[db]["loc"]) do
    if db == "quests" then loc = loc["T"] end

    local custom = server and RaijinQuest_server[db] and RaijinQuest_server[db][id] or not server
...
-- Scans localization tables for matching IDs
-- Returns table with all IDs
function RaijinQuestDatabase:GetIDByIDPart(idPart, db)
  if not RaijinQuestDB[db] then return nil end
  local ret = {}
...
function RaijinQuestDatabase:GetBestMap(maps)
...
function RaijinQuestDatabase:SearchAreaTriggerID(id, meta, maps, prio)
  ...
  for _, data in pairs(areatrigger[id]["coords"]) do
    ...
    if zone and zone > 0 then
      ...
      meta = meta or {}
      meta["spawn"] = RaijinQuest_Loc["Exploration Mark"]
      meta["spawnid"] = id
      meta["item"] = nil
      ...
      meta["level"] = RaijinQuest_Loc["N/A"]
      meta["spawntype"] = RaijinQuest_Loc["Trigger"]
      meta["respawn"] = RaijinQuest_Loc["N/A"]

      maps[zone] = maps[zone] and maps[zone] + prio or prio
      RaijinQuestMap:AddNode(meta)
    end
  end
...
-- Scans for all mobs with a specified ID
-- Adds map nodes for each and returns its map table
function RaijinQuestDatabase:SearchMobID(id, meta, maps, prio)
  ...
  for _, data in pairs(units[id]["coords"]) do
    ...
    if zone > 0 then
      -- add all gathered data
      meta = meta or {}
      meta["spawn"] = RaijinQuestDB.units.loc[id]
      meta["spawnid"] = id
      ...
      meta["spawntype"] = RaijinQuest_Loc["Unit"]
...
function RaijinQuestDatabase:SearchMetaRelation(query, meta, show)
  local maps = {}
  ...
  if RaijinQuestDB["meta"] and RaijinQuestDB["meta"][relname] then
    ...
    elseif relname == "rares" then
      meta["texture"] = RaijinQuestConfig.path.."\\img\\fav"
    end

    for id, skill in pairs(RaijinQuestDB["meta"][relname]) do
      ...
      then
        if id < 0 then
          RaijinQuestDatabase:SearchObjectID(math.abs(id), meta, maps)
        else
          RaijinQuestDatabase:SearchMobID(id, meta, maps)
        end
      end
    end
  end
...
-- Adds map nodes for each and returns its map table
function RaijinQuestDatabase:SearchMob(mob, meta, partial)
  local maps = {}

  for id in pairs(RaijinQuestDatabase:GetIDByName(mob, "units", partial)) do
    if units[id] and units[id]["coords"] then
      maps = RaijinQuestDatabase:SearchMobID(id, meta, maps)
    end
  end
...
function RaijinQuestDatabase:SearchZoneID(id, meta, maps, prio)
  ...
  if zone > 0 then
    ...
    meta = meta or {}
    meta["spawn"] = RaijinQuestDB.zones.loc[id] or UNKNOWN
    meta["spawnid"] = id

    meta["title"] = meta["quest"] or meta["item"] or meta["spawn"]
    meta["zone"]  = zone
    meta["level"] = "N/A"
    meta["spawntype"] = RaijinQuest_Loc["Area/Zone"]
    meta["respawn"] = "N/A"
    meta["x"]     = x
    meta["y"]     = y
...
-- Adds map nodes for each and returns its map table
function RaijinQuestDatabase:SearchZone(obj, meta, partial)
  local maps = {}

  for id in pairs(RaijinQuestDatabase:GetIDByName(obj, "zones", partial)) do
    if zones[id] then
      maps = RaijinQuestDatabase:SearchZoneID(id, meta, maps)
    end
  end
...
function RaijinQuestDatabase:SearchObjectSkill(id)
  if not id or not tonumber(id) then return end
  local skill, caption = nil, nil

  if (RaijinQuestDB["meta"]["herbs"][-id]) then
    skill = RaijinQuestDB["meta"]["herbs"][-id]
    caption = RaijinQuest_Loc["Herbalism"]
  elseif (RaijinQuestDB["meta"]["mines"][-id]) then
    skill = RaijinQuestDB["meta"]["mines"][-id]
    caption = RaijinQuest_Loc["Mining"]
  end
...
-- Adds map nodes for each and returns its map table
function RaijinQuestDatabase:SearchObjectID(id, meta, maps, prio)
  if not objects[id] or not objects[id]["coords"] then return maps end

  local skill, caption = RaijinQuestDatabase:SearchObjectSkill(id)
  local maps = maps or {}
  local prio = prio or 1
  ...
  for _, data in pairs(objects[id]["coords"]) do
    ...
    if zone > 0 then
      -- add all gathered data
      meta = meta or {}
      meta["spawn"] = RaijinQuestDB.objects.loc[id]
      meta["spawnid"] = id
      ...
      meta["spawntype"] = RaijinQuest_Loc["Object"]
...
-- Adds map nodes for each and returns its map table
function RaijinQuestDatabase:SearchObject(obj, meta, partial)
  local maps = {}

  for id in pairs(RaijinQuestDatabase:GetIDByName(obj, "objects", partial)) do
    if objects[id] and objects[id]["coords"] then
      maps = RaijinQuestDatabase:SearchObjectID(id, meta, maps)
    end
  end
...
function RaijinQuestDatabase:SearchItemID(id, meta, maps, allowedTypes)
  ...
  meta["itemid"] = id
  meta["item"] = RaijinQuestDB.items.loc[id]

  local minChance = tonumber(RaijinQuest_config.mindropchance)
  ...
  if items[id]["U"] and ((not allowedTypes) or allowedTypes["U"]) then
    for unit, chance in pairs(items[id]["U"]) do
      if chance >= minChance then
        ...
        meta["sellcount"] = nil
        maps = RaijinQuestDatabase:SearchMobID(unit, meta, maps)
      end
    end
  end
  ...
  if items[id]["O"] and ((not allowedTypes) or allowedTypes["O"]) then
    for object, chance in pairs(items[id]["O"]) do
      if chance >= minChance and chance > 0 then
        ...
        meta["droprate"] = chance
        meta["sellcount"] = nil
        maps = RaijinQuestDatabase:SearchObjectID(object, meta, maps)
      end
    end
  end
  ...
  if items[id]["V"] and ((not allowedTypes) or allowedTypes["V"]) then
    for unit, chance in pairs(items[id]["V"]) do
      meta["texture"] = RaijinQuestConfig.path.."\\img\\icon_vendor"
      meta["droprate"] = nil
...
-- Adds map nodes for each drop and vendor
-- Returns its map table
function RaijinQuestDatabase:SearchItem(item, meta, partial)
  local maps = {}
  local bestmap, bestscore = nil, 0

  for id in pairs(RaijinQuestDatabase:GetIDByName(item, "items", partial)) do
    maps = RaijinQuestDatabase:SearchItemID(id, meta, maps)
  end
...
-- Returns its map table
function RaijinQuestDatabase:SearchVendor(item, meta)
  local maps = {}
  local meta = meta or {}
  local bestmap, bestscore = nil, 0

  for id in pairs(RaijinQuestDatabase:GetIDByName(item, "items")) do
    meta["itemid"] = id
    meta["item"] = RaijinQuestDB.items.loc[id]
...
-- Returns its map table
function RaijinQuestDatabase:SearchQuestID(id, meta, maps)
  if not quests[id] then return end
  local maps = maps or {}
  local meta = meta or {}

  meta["questid"] = id
  meta["quest"] = RaijinQuestDB.quests.loc[id] and RaijinQuestDB.quests.loc[id].T
  meta["qlvl"] = quests[id]["lvl"]
  meta["qmin"] = quests[id]["min"]

  -- clear previous unified quest nodes
  if meta.quest then
    RaijinQuestMap.unifiedcache[meta.quest] = {}
  end

  if RaijinQuest_config["currentquestgivers"] == "1" then
    ...
    if quests[id]["start"] and not meta["qlogid"] then
      ...
      if quests[id]["start"]["U"] then
        for _, unit in pairs(quests[id]["start"]["U"]) do
          ...
          meta["texture"] = RaijinQuestConfig.path.."\\img\\available_c"
          maps = RaijinQuestDatabase:SearchMobID(unit, meta, maps, 0)
        end
      end
      ...
      if quests[id]["start"]["O"] then
        for _, object in pairs(quests[id]["start"]["O"]) do
          ...
          meta["texture"] = RaijinQuestConfig.path.."\\img\\available_c"
          maps = RaijinQuestDatabase:SearchObjectID(object, meta, maps, 0)
        end
      end
    end
      ...
      if quests[id]["end"]["U"] then
        for _, unit in pairs(quests[id]["end"]["U"]) do
          ...
          if meta["qlogid"] then
            ...
            if complete then
              meta["texture"] = RaijinQuestConfig.path.."\\img\\complete_c"
            else
              meta["texture"] = RaijinQuestConfig.path.."\\img\\complete"
            end
          else
            meta["texture"] = RaijinQuestConfig.path.."\\img\\complete_c"
          end
    ...
    if objectives then
      for i=1, objectives, 1 do
        ...
        if type == "monster" then
          ...
          for id in pairs(RaijinQuestDatabase:GetIDByName(monsterName, "units")) do
            parse_obj["U"][id] = ( objNum + 0 >= objNeeded + 0 or done ) and "DONE" or "PROG"
          end

          for id in pairs(RaijinQuestDatabase:GetIDByName(monsterName, "objects")) do
        ...
        if type == "item" then
          ...
          for id in pairs(RaijinQuestDatabase:GetIDByName(itemName, "items")) do
  ...
  if quests[id]["obj"] then
    ...
    if quests[id]["obj"]["IR"] then
      ...
      for _, item in pairs(quests[id]["obj"]["IR"]) do
        if itemreq[item] then
          requirement = RaijinQuestDB["items"]["loc"][item] or UNKNOWN
          ...
          for object, spell in pairs(itemreq[item]) do
            if object < 0 then
              ...
              RaijinQuestDatabase:TrackQuestItemDependency(requirement, id)
              if RaijinQuestDatabase.itemlist.db[requirement] then
                maps = RaijinQuestDatabase:SearchObjectID(math.abs(object), meta, maps)
              end
            elseif object > 0 then
              ...
              RaijinQuestDatabase:TrackQuestItemDependency(requirement, id)
              if RaijinQuestDatabase.itemlist.db[requirement] then
                maps = RaijinQuestDatabase:SearchMobID(math.abs(object), meta, maps)
              end
            end
          end
        end
    ...
    if quests[id]["obj"]["I"] then
      for _, item in pairs(quests[id]["obj"]["I"]) do
        if not parse_obj["I"][item] or parse_obj["I"][item] ~= "DONE" then
          ...
          maps = RaijinQuestDatabase:SearchItemID(item, meta, maps)
        end
      end
    end
    ...
    if quests[id]["obj"]["A"] then
      for _, areatrigger in pairs(quests[id]["obj"]["A"]) do
        ...
        maps = RaijinQuestDatabase:SearchAreaTriggerID(areatrigger, meta, maps)
      end
    end
    ...
    if quests[id]["obj"]["Z"] then
      for _, zone in pairs(quests[id]["obj"]["Z"]) do
        ...
        maps = RaijinQuestDatabase:SearchZoneID(zone, meta, maps)
      end
    end
  end
  ...
  if RaijinQuestMap.nodes[addon] then
    for map in pairs(RaijinQuestMap.nodes[addon]) do
      if meta.quest and RaijinQuestMap.unifiedcache[meta.quest] and RaijinQuestMap.unifiedcache[meta.quest][map] then
        for hash, data in pairs(RaijinQuestMap.unifiedcache[meta.quest][map]) do
          meta = data.meta
          meta["title"] = meta["quest"]
          meta["cluster"] = true
          meta["zone"]  = map

          local icon = RaijinQuest_config["clustermono"] == "1" and "_mono" or ""

          if meta.item then
            meta["x"], meta["y"], meta["priority"] = getcluster(data.coords, meta["quest"]..hash..map)
            meta["texture"] = RaijinQuestConfig.path.."\\img\\cluster_item" .. icon
            RaijinQuestMap:AddNode(meta, true)
          elseif meta.spawntype and meta.spawntype == RaijinQuest_Loc["Unit"] and meta.spawn and not meta.itemreq then
            meta["x"], meta["y"], meta["priority"] = getcluster(data.coords, meta["quest"]..hash..map)
            meta["texture"] = RaijinQuestConfig.path.."\\img\\cluster_mob" .. icon
            RaijinQuestMap:AddNode(meta, true)
          else
            meta["x"], meta["y"], meta["priority"] = getcluster(data.coords, meta["quest"]..hash..map)
            meta["texture"] = RaijinQuestConfig.path.."\\img\\cluster_misc" .. icon
            RaijinQuestMap:AddNode(meta, true)
          end
        end
      end
...
-- Returns its map table
function RaijinQuestDatabase:SearchQuest(quest, meta, partial)
  local maps = {}

  for id in pairs(RaijinQuestDatabase:GetIDByName(quest, "quests", partial)) do
...
function RaijinQuestDatabase:QuestFilter(id, plevel, pclass, prace)
  -- hide active quest
  if RaijinQuest.questlog[id] then return end

  -- hide completed quests
  if RaijinQuest_history[id] then return end

  -- hide broken quests without names
  if not RaijinQuestDB.quests.loc[id] or not RaijinQuestDB.quests.loc[id].T then return end
  ...
  if quests[id]["pre"] then
    ...
    for _, prequest in pairs(quests[id]["pre"]) do
      if RaijinQuest_history[prequest] then
        one_complete = true
      end
    end
  ...
  if quests[id]["skill"] and not RaijinQuestDatabase:GetPlayerSkill(quests[id]["skill"]) then return end

  -- hide lowlevel quests
  if quests[id]["lvl"] and quests[id]["lvl"] < plevel - 4 and RaijinQuest_config["showlowlevel"] == "0" then return end

  -- hide highlevel quests (or show those that are 3 levels above)
  if quests[id]["min"] and quests[id]["min"] > plevel + ( RaijinQuest_config["showhighlevel"] == "1" and 3 or 0 ) then return end

  -- hide event quests
  if quests[id]["event"] and RaijinQuest_config["showfestival"] == "0" then return end
...
-- Returns its map table
function RaijinQuestDatabase:SearchQuests(meta, maps)
  local level, minlvl, maxlvl, race, class, prof, festival
  local maps = maps or {}
  local meta = meta or {}

  local plevel = UnitLevel("player")
  local rqaction = UnitFactionGroup("player")
  if rqaction == "Horde" then
    rqaction = "H"
  elseif rqaction == "Alliance" then
    rqaction = "A"
  else
    rqaction = "GM"
  end
  ...
  local prace = RaijinQuestDatabase:GetBitByRace(race)
  local _, class = UnitClass("player")
  local pclass = RaijinQuestDatabase:GetBitByClass(class)

  for id in pairs(quests) do
    if RaijinQuestDatabase:QuestFilter(id, plevel, pclass, prace) then
      -- set metadata
      meta["quest"] = ( RaijinQuestDB.quests.loc[id] and RaijinQuestDB.quests.loc[id].T ) or UNKNOWN
      meta["questid"] = id
      meta["texture"] = RaijinQuestConfig.path.."\\img\\available_c"
      ...
      if quests[id]["min"] and quests[id]["min"] > plevel then
        meta["texture"] = RaijinQuestConfig.path.."\\img\\available"
        meta["vertex"] = { 1, .6, .6 }
        meta["layer"] = 2
      end
      ...
      if quests[id]["start"] then
        ...
        if quests[id]["start"]["U"] then
          meta["QTYPE"] = "NPC_START"
          for _, unit in pairs(quests[id]["start"]["U"]) do
            if units[unit] and strfind(units[unit]["fac"] or rqaction, rqaction) then
              maps = RaijinQuestDatabase:SearchMobID(unit, meta, maps)
            end
          end
        end
        ...
        if quests[id]["start"]["O"] then
          meta["QTYPE"] = "OBJECT_START"
          for _, object in pairs(quests[id]["start"]["O"]) do
            if objects[object] and strfind(objects[object]["fac"] or rqaction, rqaction) then
              maps = RaijinQuestDatabase:SearchObjectID(object, meta, maps)
            end
          end
        end
...
function RaijinQuestDatabase:FormatQuestText(questText)
...
function RaijinQuestDatabase:GetQuestIDs(qid)
  ...
  if RaijinQuest_questcache[identifier] and RaijinQuest_questcache[identifier][1] then
    return RaijinQuest_questcache[identifier]
  end
  ...
  for id, data in pairs(RaijinQuestDB["quests"]["loc"]) do
  ...
  if tcount == 0 and title then
    ...
    if not ttitle then
      ...
      if not RaijinQuestDatabase.localized then
        -- skip cache if locale-checks are still running
        return { title }
      else
        -- flag quest as unknown and return
        RaijinQuest_questcache[identifier] = { title }
        return RaijinQuest_questcache[identifier]
      end
    else
  ...
  for id, data in pairs(RaijinQuestDB["quests"]["loc"]) do
    ...
    if quests[id] and data.T and data.T == title then
      ...
      if tcount > 1 then
        ...
        score = score + max(24 - lev(RaijinQuestDatabase:FormatQuestText(RaijinQuestDB.quests.loc[id]["O"]), objective, 24),0)

        -- check description and calculate score
        score = score + max(24 - lev(RaijinQuestDatabase:FormatQuestText(RaijinQuestDB.quests.loc[id]["D"]), text, 24),0)
      end
  ...
  -- cache for next time
  RaijinQuest_questcache[identifier] = results[best]
  return results[best]
end

-- browser search related defaults and values
RaijinQuestDatabase.lastSearchQuery = ""
RaijinQuestDatabase.lastSearchResults = {["items"] = {}, ["quests"] = {}, ["objects"] = {}, ["units"] = {}}
...
function RaijinQuestDatabase:BrowserSearch(query, searchType)
  ...
  if (queryLength >= minChars) or (queryNumber and (queryLength >= minInts)) then -- make sure this is no fav display
    if ((queryLength > minChars) or (queryNumber and (queryLength > minInts)))
       and (RaijinQuestDatabase.lastSearchQuery ~= "" and queryLength > strlen(RaijinQuestDatabase.lastSearchQuery))
    then
      -- there are previous search results to use
      local searchDatabase = RaijinQuestDatabase.lastSearchResults[searchType]
      -- iterate the last search
      for id, _ in pairs(searchDatabase) do
        local dbLocale = RaijinQuestDB[searchType]["loc"][id]
        if (dbLocale) then
          local compare
          local search = query
          if (queryNumber) then
      ...
      if (queryNumber) then
        results = RaijinQuestDatabase:GetIDByIDPart(query, searchType)
      else
        results = RaijinQuestDatabase:GetIDByName(query, searchType, true)
      end
      local resultCount = 0
    ...
    return RaijinQuestBrowser_fav[searchType], -1
  end
end
...
local function LoadCustomData(always)
  ...
  local icount = 0
  for _,_ in pairs(RaijinQuest_server["items"]) do
    icount = icount + 1
  end

  if icount > 0 or always then
    for id, name in pairs(RaijinQuest_server["items"]) do
      RaijinQuestDB["items"]["loc"][id] = name
    end
    DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest: |cff33ffcc" .. icount .. "|cffffffff " .. RaijinQuest_Loc["custom items loaded."])
  end
end

local RaijinQuestServerScan = CreateFrame("Frame", "rqServerItemScan", UIParent)
RaijinQuestServerScan:SetWidth(200)
RaijinQuestServerScan:SetHeight(100)
RaijinQuestServerScan:SetPoint("TOP", 0, 0)
RaijinQuestServerScan:Hide()

RaijinQuestServerScan.scanID = 1
RaijinQuestServerScan.max = 100000
RaijinQuestServerScan.perloop = 100

RaijinQuestServerScan.header = RaijinQuestServerScan:CreateFontString("Caption", "LOW", "GameFontWhite")
RaijinQuestServerScan.header:SetFont(STANDARD_TEXT_FONT, 13, "OUTLINE")
RaijinQuestServerScan.header:SetJustifyH("CENTER")
RaijinQuestServerScan.header:SetPoint("CENTER", 0, 0)

RaijinQuestServerScan:RegisterEvent("VARIABLES_LOADED")
RaijinQuestServerScan:SetScript("OnEvent", function()
  RaijinQuest_server = RaijinQuest_server or { }
  RaijinQuest_server["items"] = RaijinQuest_server["items"] or {}
  LoadCustomData()
end)

RaijinQuestServerScan:SetScript("OnHide", function()
  ItemRefTooltip:Show()
  LoadCustomData(true)
end)

RaijinQuestServerScan:SetScript("OnShow", function()
  this.scanID = 1
  RaijinQuest_server["items"] = {}
  DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccpf|cffffffffQuest: " .. RaijinQuest_Loc["Server scan started..."])
end)

local ignore, custom_id, custom_skip = {}, nil, nil
RaijinQuestServerScan:SetScript("OnUpdate", function()
  ...
  for i=this.scanID,this.scanID+this.perloop do
    RaijinQuestServerScan.header:SetText(RaijinQuest_Loc["Scanning server for items..."] .. " " .. string.format("%.1f",100*i/this.max) .. "%")
    ...
    if ItemRefTooltipTextLeft1 and ItemRefTooltipTextLeft1:IsVisible() then
      ...
      if not RaijinQuestDB["items"]["loc"][i] and not ignore[i] then
        RaijinQuest_server["items"][i] = name
      end
    end
  end
...
function RaijinQuestDatabase:ScanServer()
  RaijinQuestServerScan:Show()
end

function RaijinQuestDatabase:QueryServer()
  ...
  local function OnQuestQueryComplete()
    ...
    if type(completedQuests) == "table" then
      for questID, _ in pairs(completedQuests) do
        RaijinQuest_history[questID] = { time(), UnitLevel("player") }
      end
