...
-- fake the RaijinQuest minimap node names to Gatherer names,
-- if any minimap-breaking addon collector is found.
local nodename = "rqMiniMapPin"
...
local controlkey = CreateFrame("Frame", "RaijinQuestControlKey", UIParent)
controlkey:SetScript("OnUpdate", function()
  if ( this.throttle or .2) > GetTime() then return else this.throttle = GetTime() + .2 end
  if WorldMapFrame:IsShown() and MouseIsOver(WorldMapFrame) or MouseIsOver(RaijinQuestMap.drawlayer) then
...
local minimap_sizes = RaijinQuestDB["minimap"]

local unifiedcache = {}

-- used to store/cache combined meta data across nodes of
-- the same kind to avoid duplicating data for each pin
-- the objects here get directly attached to the RaijinQuestMap nodes
local similar_nodes = {}
...
local layers = {
  -- regular icons
  [RaijinQuestConfig.path.."\\img\\available"]          = 1,
  [RaijinQuestConfig.path.."\\img\\available_c"]        = 2,
  [RaijinQuestConfig.path.."\\img\\complete"]           = 3,
  [RaijinQuestConfig.path.."\\img\\complete_c"]         = 4,
  [RaijinQuestConfig.path.."\\img\\icon_vendor"]        = 5,
  [RaijinQuestConfig.path.."\\img\\fav"]                = 6,

  -- cluster textures
  [RaijinQuestConfig.path.."\\img\\cluster_item"]       = 9,
  [RaijinQuestConfig.path.."\\img\\cluster_mob"]        = 9,
  [RaijinQuestConfig.path.."\\img\\cluster_misc"]       = 9,
  [RaijinQuestConfig.path.."\\img\\cluster_mob_mono"]   = 9,
  [RaijinQuestConfig.path.."\\img\\cluster_item_mono"]  = 9,
  [RaijinQuestConfig.path.."\\img\\cluster_misc_mono"]  = 9,
}
...
local function str2rgb(text)
  ...
  if RaijinQuest_colors[text] then return unpack(RaijinQuest_colors[text]) end
...
RaijinQuestMap = CreateFrame("Frame", "RaijinQuestMap", WorldFrame)
RaijinQuestMap.str2rgb = str2rgb
RaijinQuestMap.tooltips = {}
RaijinQuestMap.nodes = {}
RaijinQuestMap.pins = {}
RaijinQuestMap.mpins = {}
RaijinQuestMap.drawlayer = Minimap
RaijinQuestMap.unifiedcache = unifiedcache

RaijinQuestMap.minimap_sizes = minimap_sizes

RaijinQuestMap.tooltip = CreateFrame("Frame" , "RaijinQuestMapTooltip", GameTooltip)
RaijinQuestMap.tooltip:SetScript("OnShow", function()
  local focus = GetMouseFocus()
  -- abort on RaijinQuest nodes
  ...
  if RaijinQuest_config.showtooltips == "0" then return end

  local name = getglobal("GameTooltipTextLeft1") and getglobal("GameTooltipTextLeft1"):GetText() or "__NONE__"
  local zone = RaijinQuestMap:GetMapID(GetCurrentMapContinent(), GetCurrentMapZone())

  -- remove all colors from received tooltip text
  name = string.gsub(name, "|c%x%x%x%x%x%x%x%x", "")
  name = string.gsub(name, "|r", "")

  if RaijinQuestMap.tooltips[name] and RaijinQuestMap.tooltips[name] then
    for title, obj in pairs(RaijinQuestMap.tooltips[name]) do
      if obj[zone] then
        RaijinQuestMap:ShowTooltip(obj[zone], GameTooltip)
        GameTooltip:Show()
      end
    end
  end
...
function RaijinQuestMap:HasMinimap()
  return true
end

function RaijinQuestMap.tooltip:GetColor(min, max)
  local max = max or 1
...
function RaijinQuestMap:HexDifficultyColor(level, force)
...
function RaijinQuestMap:ShowTooltip(meta, tooltip)
  local catch = nil
  local catch_obj = nil
  ...
  if meta["quest"] then
          ...
          for i=1, objectives, 1 do
            ...
            if type == "monster" then
              ...
              if monsterName and meta["spawn"] == monsterName then
                catch_obj = true
                local r,g,b = RaijinQuestMap.tooltip:GetColor(objNum, objNeeded)
            ...
            elseif table.getn(meta["item"]) > 0 and type == "item" and meta["droprate"] then
              ...
              for mid, item in pairs(meta["item"]) do
                if item == itemName then
                  catch_obj = true
                  local r,g,b = RaijinQuestMap.tooltip:GetColor(objNum, objNeeded)
                  local dr,dg,db = RaijinQuestMap.tooltip:GetColor(tonumber(meta["droprate"]), 100)
            ...
            elseif table.getn(meta["item"]) > 0 and type == "item" and meta["sellcount"] then
              ...
              for mid, item in pairs(meta["item"]) do
                if item == itemName then
                  ...
                  tooltip:AddLine("|cffaaaaaa- |r" .. RaijinQuest_Loc["Buy"] .. ": " .. itemName .. ": " .. objNum .. "/" .. objNeeded .. sellcount, r, g, b)
                end
              end
            end
          end
    ...
    if not catch_obj then
      ...
      if meta["item"] and meta["item"][1] and meta["sellcount"] then
        for mid, item in pairs(meta["item"]) do
          catchFallback = true
          local sellcount = tonumber(meta["sellcount"]) > 0 and " |cff555555[|cffcccccc" .. meta["sellcount"] .. "x" .. "|cff555555]" or ""
          tooltip:AddLine("|cffaaaaaa- |r" .. RaijinQuest_Loc["Buy"] .. ": " .. item .. sellcount, .7, .7, .7)
        end
      end

      if not catchFallback and meta["spawn"] and not meta["texture"] then
        catchFallback = true
        tooltip:AddLine("|cffaaaaaa- |r" .. (meta["spawntype"] and meta["spawntype"] == "Trigger" and RaijinQuest_Loc["Explore"] or meta["spawn"]), .7,.7,.7)
      end

      if not catchFallback and meta["texture"] and meta["qlvl"] then
        local texts = meta["questid"] and RaijinQuestDB["quests"]["loc"][meta["questid"]] or nil

        if texts and texts["O"] and texts["O"] ~= "" then
          tooltip:AddLine(RaijinQuestDatabase:FormatQuestText(texts["O"]),1,1,.9,true)
        end

        local qlvlstr = RaijinQuest_Loc["Level"] .. ": " .. RaijinQuestMap:HexDifficultyColor(meta["qlvl"]) .. meta["qlvl"] .. "|r"
        local qminstr = meta["qmin"] and " / " .. RaijinQuest_Loc["Required"] .. ": " .. RaijinQuestMap:HexDifficultyColor(meta["qmin"], true) .. meta["qmin"] .. "|r"  or ""
    ...
    if meta["sellcount"] then
      ...
      local sellcount = tonumber(meta["sellcount"]) > 0 and " |cff555555[|cffcccccc" .. meta["sellcount"] .. "x" .. "|cff555555]" or ""
      tooltip:AddLine(RaijinQuest_Loc["Vendor"] .. ": " .. item .. sellcount, 1,1,1)
    elseif meta["item"][1] then
      local item = meta["itemlink"] or "[" .. meta["item"][1] .. "]"
      local r,g,b = RaijinQuestMap.tooltip:GetColor(tonumber(meta["droprate"]), 100)
      tooltip:AddLine("|cffffffff" .. RaijinQuest_Loc["Loot"] .. ": " .. item ..  " |cff555555[|r" .. meta["droprate"] .. "%|cff555555]", r,g,b)
    end
  end
...
function RaijinQuestMap:GetMapNameByID(id)
  id = tonumber(id)
  return RaijinQuestDB["zones"]["loc"][id] or nil
end

function RaijinQuestMap:GetMapIDByName(search)
  for id, name in pairs(RaijinQuestDB["zones"]["loc"]) do
    if name == search then
      return id
    end
  end
...
function RaijinQuestMap:ShowMapID(map)
  if map then
...
function RaijinQuestMap:SetMapByID(id)
  local search = RaijinQuestDB["zones"]["loc"][id]
...
function RaijinQuestMap:GetMapID(cid, mid)
  ...
  local name = list[mid]
  local id = RaijinQuestMap:GetMapIDByName(name)
  id = id or customids[GetMapInfo()]

  return id
end

function RaijinQuestMap:AddNode(meta)
  if not meta then return end
  if not meta["zone"] then return end
  if not meta["title"] then return end

  meta["description"] = RaijinQuestDatabase:BuildQuestDescription(meta)
  ...
  if not RaijinQuestMap.nodes[addon] then RaijinQuestMap.nodes[addon] = {} end
  if not RaijinQuestMap.nodes[addon][map] then RaijinQuestMap.nodes[addon][map] = {} end
  if not RaijinQuestMap.nodes[addon][map][coords] then RaijinQuestMap.nodes[addon][map][coords] = {} end

  -- skip early on existing nodes
  if RaijinQuestMap.nodes[addon][map][coords][title] then
    if item and table.getn(RaijinQuestMap.nodes[addon][map][coords][title].item) > 0 then
      -- check if item already exists
      for id, name in pairs(RaijinQuestMap.nodes[addon][map][coords][title].item) do
        if name == item then return end
      end

      -- add new item and exit
      table.insert(RaijinQuestMap.nodes[addon][map][coords][title].item, item)
      return
    end

    if RaijinQuestMap.nodes[addon][map][coords][title] and RaijinQuestMap.nodes[addon][map][coords][title].layer and layer and
     RaijinQuestMap.nodes[addon][map][coords][title].layer >= layer then
  ...
  RaijinQuestMap.nodes[addon][map][coords][title] = similar_nodes[sindex]
  ...
  if spawn and title then
    RaijinQuestMap.tooltips[spawn] = RaijinQuestMap.tooltips[spawn] or {}
    RaijinQuestMap.tooltips[spawn][title] = RaijinQuestMap.tooltips[spawn][title] or {}
    RaijinQuestMap.tooltips[spawn][title][map] = RaijinQuestMap.tooltips[spawn][title][map] or similar_nodes[sindex]
  end

  RaijinQuestMap.queue_update = GetTime()
end

function RaijinQuestMap:GetNodes(addon, title)
  local nodes = {}

  if title and RaijinQuestMap.nodes[addon] then
    for map, foo in pairs(RaijinQuestMap.nodes[addon]) do
      for coords, node in pairs(RaijinQuestMap.nodes[addon][map]) do
        if RaijinQuestMap.nodes[addon][map][coords][title] then
          table.insert(nodes, RaijinQuestMap.nodes[addon][map][coords][title])
        end
      end
    end
  end
...
function RaijinQuestMap:DeleteNode(addon, title)
  -- remove tooltips
  if not addon then
    RaijinQuestMap.tooltips = {}
  else
    for mk, mv in pairs(RaijinQuestMap.tooltips) do
      for tk, tv in pairs(mv) do
        if ( title and tk == title ) or ( not title and tv.addon == addon ) then
          RaijinQuestMap.tooltips[mk][tk] = nil
        end
      end
    end
  end
  ...
  elseif not title then
    RaijinQuestMap.nodes[addon] = {}
  elseif RaijinQuestMap.nodes[addon] then
    for map, foo in pairs(RaijinQuestMap.nodes[addon]) do
      for coords, node in pairs(RaijinQuestMap.nodes[addon][map]) do
        if RaijinQuestMap.nodes[addon][map][coords][title] then
          RaijinQuestMap.nodes[addon][map][coords][title] = nil
          if IsEmpty(RaijinQuestMap.nodes[addon][map][coords]) then
            RaijinQuestMap.nodes[addon][map][coords] = nil
          end
        end
      end
    end
...
function RaijinQuestMap:NodeClick()
  if IsShiftKeyDown() then
    if this.questid and this.texture and this.layer < 5 then
      -- mark questnode as done
      RaijinQuest_history[this.questid] = { time(), UnitLevel("player") }
    end

    if this.node and this.title and this.node[this.title] then
      -- delete node from map
      RaijinQuestMap:DeleteNode(this.node[this.title].addon, this.title)
    end

    RaijinQuest.updateQuestGivers = true
  elseif this.texture and RaijinQuest.route and
   (( RaijinQuest_config["routecluster"] == "1" and this.layer >= 9 ) or
    ( RaijinQuest_config["routeender"] == "1" and this.layer == 4) or
    ( RaijinQuest_config["routestarter"] == "1" and this.layer == 1) or
    ( RaijinQuest_config["routestarter"] == "1" and this.layer == 2))
  then
    -- set as arrow target priority
    RaijinQuest.route.SetTarget((not RaijinQuest.route.IsTarget(this) and this))
    RaijinQuestMap.queue_update = GetTime()
  else
    -- switch color
    RaijinQuest_colors[this.color] = { str2rgb(this.color .. GetTime()) }
    RaijinQuestMap.queue_update = GetTime()
  end
end

function RaijinQuestMap:NodeEnter()
  ...
  tooltip:SetText(this.spawn..(RaijinQuest_config.showids == "1" and " |cffcccccc("..this.spawnid..")|r" or ""), .3, 1, .8)
  tooltip:AddDoubleLine(RaijinQuest_Loc["Level"] .. ":", (this.level or UNKNOWN), .8,.8,.8, 1,1,1)
  tooltip:AddDoubleLine(RaijinQuest_Loc["Type"] .. ":", (this.spawntype or UNKNOWN), .8,.8,.8, 1,1,1)
  tooltip:AddDoubleLine(RaijinQuest_Loc["Respawn"] .. ":", (this.respawn or UNKNOWN), .8,.8,.8, 1,1,1)

  for title, meta in pairs(this.node) do
    RaijinQuestMap:ShowTooltip(meta, tooltip)
  end

  -- add tooltip help if setting is enabled
  if RaijinQuest_config["tooltiphelp"] == "1" then
    local text = RaijinQuest_Loc["Use <Shift>-Click To Remove Nodes"]

    if this.cluster then
      text = RaijinQuest_Loc["Hold <Ctrl> To Hide Cluster"]
    elseif tooltip == GameTooltip then
      text = RaijinQuest_Loc["Hold <Ctrl> To Hide Minimap Nodes"]
    elseif not this.texture then
      text = RaijinQuest_Loc["Click Node To Change Color"]
    elseif this.questid and this.texture and this.layer < 5 then
      text = RaijinQuest_Loc["Use <Shift>-Click To Mark Quest As Done"]
    end
  ...
  RaijinQuestMap.highlight = RaijinQuest_config["mouseover"] == "1" and this.title
end

function RaijinQuestMap:NodeLeave()
  ...
  local tooltip = this:GetParent() == WorldMapButton and WorldMapTooltip or GameTooltip
  tooltip:Hide()
  RaijinQuestMap.highlight = nil
end

function RaijinQuestMap:BuildNode(name, parent)
  local f = CreateFrame("Button", name, parent)

  if parent == WorldMapButton then
    f.defalpha = tonumber(RaijinQuest_config["worldmaptransp"]) or 1
    f.defsize = 16
  else
    f.defalpha = tonumber(RaijinQuest_config["minimaptransp"]) or 1
    f.defsize = 16
    f.minimap = true
  end
  ...
  f:SetScript("OnEnter", RaijinQuestMap.NodeEnter)
  f:SetScript("OnLeave", RaijinQuestMap.NodeLeave)

  f.tex = f:CreateTexture(nil, "BACKGROUND")
  f.tex:SetAllPoints(f)

  f.hl = f:CreateTexture(nil, "BORDER")
  f.hl:SetTexture(RaijinQuestConfig.path.."\\img\\track")
...
RaijinQuestMap.highlightdb = {}
function RaijinQuestMap:UpdateNode(frame, node, color, obj)
  -- clear node to title association table
  if RaijinQuestMap.highlightdb[frame] then
    for k,v in pairs(RaijinQuestMap.highlightdb[frame]) do
      RaijinQuestMap.highlightdb[frame][k] = nil
    end
  else
    RaijinQuestMap.highlightdb[frame] = {}
  end
  ...
  for title, tab in pairs(node) do
    RaijinQuestMap.highlightdb[frame][title] = true
    ...
    if tab.spawn and ( tab.layer > frame.layer or not frame.spawn ) then
      ...
      if RaijinQuest_config["spawncolors"] == "1" then
  ...
  if ( frame.updateColor or frame.updateTexture or not frame.tex:GetTexture() ) and not frame.texture then
    ...
    if obj == "minimap" and RaijinQuest_config["cutoutminimap"] == "1" then
      frame.tex:SetTexture(RaijinQuestConfig.path.."\\img\\nodecut")
      frame.tex:SetVertexColor(r,g,b,1)
    elseif obj ~= "minimap" and RaijinQuest_config["cutoutworldmap"] == "1" then
      frame.tex:SetTexture(RaijinQuestConfig.path.."\\img\\nodecut")
      frame.tex:SetVertexColor(r,g,b,1)
    elseif frame.title and RaijinQuest.icons[frame.title] then
      frame.tex:SetTexture(RaijinQuest.icons[frame.title])
  ...
  if frame.updateTexture or frame.updateVertex or frame.updateColor or frame.updateLayer then
    frame:SetScript("OnClick", (frame.func or RaijinQuestMap.NodeClick))
  end

  local highlight = frame.texture and RaijinQuestMap.highlightdb[frame][RaijinQuestMap.highlight] and true or nil
  local target = frame.texture and RaijinQuest.route and RaijinQuest.route.IsTarget(frame) or nil
...
function RaijinQuestMap:UpdateNodes()
  RaijinQuest:Debug("Update Nodes")

  local color = RaijinQuest_config["spawncolors"] == "1" and "spawn" or "title"
  local map = RaijinQuestMap:GetMapID(GetCurrentMapContinent(), GetCurrentMapZone())
  local i = 1

  -- reset tracker
  RaijinQuest.tracker.Reset()

  -- reset route
  RaijinQuest.route:Reset()

  -- refresh all nodes
  for addon, _ in pairs(RaijinQuestMap.nodes) do
    if RaijinQuestMap.nodes[addon][map] then
      for coords, node in pairs(RaijinQuestMap.nodes[addon][map]) do
        if not RaijinQuestMap.pins[i] then
          RaijinQuestMap.pins[i] = RaijinQuestMap:BuildNode("RaijinQuestMapPin" .. i, WorldMapButton)
        end

        RaijinQuestMap:UpdateNode(RaijinQuestMap.pins[i], node, color)

        -- set position
        local _, _, x, y = strfind(coords, "(.*)|(.*)")

        -- write points to the route plan
        if ( RaijinQuest_config["routecluster"] == "1" and RaijinQuestMap.pins[i].layer >= 9 ) or
          ( RaijinQuest_config["routeender"] == "1" and RaijinQuestMap.pins[i].layer == 4) or
          ( RaijinQuest_config["routestarter"] == "1" and RaijinQuestMap.pins[i].layer == 1 and RaijinQuestMap.pins[i].texture) or
          ( RaijinQuest_config["routestarter"] == "1" and RaijinQuestMap.pins[i].layer == 2) or
          RaijinQuestMap.pins[i].arrow == true
        then
          RaijinQuest.route:AddPoint({ x, y, RaijinQuestMap.pins[i] })
        end

        -- hide cluster nodes if set
        if RaijinQuest_config["showcluster"] == "0" and RaijinQuestMap.pins[i].cluster then
          RaijinQuestMap.pins[i]:Hide()
        -- hide individual quest spawns
        elseif RaijinQuest_config["showspawn"] == "0" and addon == "PFQUEST" and not RaijinQuestMap.pins[i].texture then
          RaijinQuestMap.pins[i]:Hide()
        else
          -- populate quest list on map
          for title, node in pairs(RaijinQuestMap.pins[i].node) do
            RaijinQuest.tracker.ButtonAdd(title, node)
          end

          x = x / 100 * WorldMapButton:GetWidth()
          y = y / 100 * WorldMapButton:GetHeight()

          RaijinQuestMap.pins[i]:ClearAllPoints()
          RaijinQuestMap.pins[i]:SetPoint("CENTER", WorldMapButton, "TOPLEFT", x, -y)

          RaijinQuestMap.pins[i]:Show()
        end
  ...
  -- hide remaining pins
  for j=i, table.getn(RaijinQuestMap.pins) do
    if RaijinQuestMap.pins[j] then RaijinQuestMap.pins[j]:Hide() end
  end
end

local coord_cache = {}
function RaijinQuestMap:UpdateMinimap()
  -- check for disabled minimap nodes
  if RaijinQuest_config["minimapnodes"] == "0" then
    return
  end

  -- hide all minimap nodes while shift is pressed
  if controlkey.pressed and MouseIsOver(RaijinQuestMap.drawlayer) then
    this.xPlayer = nil

    for id, pin in pairs(RaijinQuestMap.mpins) do
      pin:Hide()
    end
  ...
  if xPlayer == 0 and yPlayer == 0 then
    for pins, pin in pairs(RaijinQuestMap.mpins) do pin:Hide() end
    return
  end

  local mZoom = RaijinQuestMap.drawlayer:GetZoom()
  ...
  local color = RaijinQuest_config["spawncolors"] == "1" and "spawn" or "title"
  local mapID = RaijinQuestMap:GetMapIDByName(GetRealZoneText())
  ...
  local xDraw = RaijinQuestMap.drawlayer:GetWidth() / xScale / 100
  local yDraw = RaijinQuestMap.drawlayer:GetHeight() / yScale / 100

  local i = 1

  -- refresh all nodes
  for addon, data in pairs(RaijinQuestMap.nodes) do
    -- hide minimap nodes in continent view
    if data[mapID] and minimap_sizes[mapID] and RaijinQuestMap:HasMinimap(mapID) then
      for coords, node in pairs(data[mapID]) do
        ...
        if RaijinQuestCompat.rotateMinimap then
          ...
          local sinFacing = sin(RaijinQuestCompat.GetPlayerFacing())
          local cosFacing = cos(RaijinQuestCompat.GetPlayerFacing())
        ...
        if pfUI.minimap then
          display = ( abs(xPos) + 8 < RaijinQuestMap.drawlayer:GetWidth() / 2 and abs(yPos) + 8 < RaijinQuestMap.drawlayer:GetHeight()/2 ) and true or nil
        else
          local distance = sqrt(xPos * xPos + yPos * yPos)
          display = ( distance + 8 < RaijinQuestMap.drawlayer:GetWidth() / 2 ) and true or nil
        end

        if display then
          if not RaijinQuestMap.mpins[i] then
            RaijinQuestMap.mpins[i] = RaijinQuestMap:BuildNode(nodename .. i, RaijinQuestMap.drawlayer)
          end

          RaijinQuestMap:UpdateNode(RaijinQuestMap.mpins[i], node, color, "minimap")

          RaijinQuestMap.mpins[i].hl:Hide()

          if RaijinQuest_config["showclustermini"] == "0" and RaijinQuestMap.mpins[i].cluster then
            RaijinQuestMap.mpins[i]:Hide()
          elseif RaijinQuest_config["showspawnmini"] == "0" and addon == "PFQUEST" and not RaijinQuestMap.mpins[i].texture then
            RaijinQuestMap.mpins[i]:Hide()
          else
            RaijinQuestMap.mpins[i]:ClearAllPoints()
            RaijinQuestMap.mpins[i]:SetPoint("CENTER", RaijinQuestMap.drawlayer, "CENTER", xPos, -yPos)
            RaijinQuestMap.mpins[i]:Show()
          end
  ...
  -- hide remaining pins
  for j=i, table.getn(RaijinQuestMap.mpins) do
    if RaijinQuestMap.mpins[j] then RaijinQuestMap.mpins[j]:Hide() end
  end
end

local zone, last_zone
RaijinQuestMap:RegisterEvent("ZONE_CHANGED")
RaijinQuestMap:RegisterEvent("ZONE_CHANGED_NEW_AREA")
RaijinQuestMap:RegisterEvent("MINIMAP_ZONE_CHANGED")
RaijinQuestMap:RegisterEvent("WORLD_MAP_UPDATE")
RaijinQuestMap:SetScript("OnEvent", function()
  -- save current zone
...
RaijinQuestMap:SetScript("OnUpdate", function()
  -- handle highlights and animations
  if RaijinQuestMap.queue_update or transition or RaijinQuestMap.highlight ~= hlstate or shiftstate ~= hidecluster then
    hlstate, shiftstate, transition = RaijinQuestMap.highlight, hidecluster, nil
    fps = math.max(.2, GetFramerate() / 30)

    for frame, data in pairs(RaijinQuestMap.highlightdb) do
      local highlight = RaijinQuestMap.highlightdb[frame][RaijinQuestMap.highlight] and true or nil
      ...
      elseif not highlight and RaijinQuestMap.highlight then
        -- fade node
        transition = frame:Animate(frame.defsize, tonumber(RaijinQuest_config["nodefade"]) or 0.3, fps) or transition
  ...
  if RaijinQuestMap.queue_update and RaijinQuestMap.queue_update + .25 < GetTime() then
    RaijinQuestMap.queue_update = nil
    RaijinQuestMap:UpdateNodes()
  end
...
if compat.client >= 30300 then
  ...
  local rqHookWorldMapQuestFrame_OnMouseUp = WorldMapQuestFrame_OnMouseUp
  WorldMapQuestFrame_OnMouseUp = function(self)
    rqHookWorldMapQuestFrame_OnMouseUp(self)
    ...
    if not IsShiftKeyDown() then
      ...
      if title then
        if previousTitle == title then
          ...
          RaijinQuestMap.highlight = title
          previousTitle = title
          RaijinQuestMap.queue_update = GetTime()
        end
      end
