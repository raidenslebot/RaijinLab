...
local function DrawLine(path,x,y,nx,ny,hl,minimap)
  ...
  if minimap then
    ...
    local mapWidth = RaijinQuestMap.minimap_sizes[mapID] and RaijinQuestMap.minimap_sizes[mapID][1] or 0
    local mapHeight = RaijinQuestMap.minimap_sizes[mapID] and RaijinQuestMap.minimap_sizes[mapID][2] or 0

    -- calculate drawlayer size
    xdraw = RaijinQuestMap.drawlayer:GetWidth() / (mapZoom / mapWidth) / 100
    ydraw = RaijinQuestMap.drawlayer:GetHeight() / (mapZoom / mapHeight) / 100
  ...
  for i=(minimap and 1 or 2), dots-(minimap and 1 or 2) do
    ...
    if minimap then
      ...
      if pfUI.minimap then
        display = ( abs(xpos) + 1 < RaijinQuestMap.drawlayer:GetWidth() / 2 and abs(ypos) + 1 < RaijinQuestMap.drawlayer:GetHeight()/2 ) and true or nil
      else
        local distance = sqrt(xpos * xpos + ypos * ypos)
        display = ( distance + 1 < RaijinQuestMap.drawlayer:GetWidth() / 2 ) and true or nil
      end
    else
    ...
    if display then
      ...
      path[nline] = path[nline] or (minimap and RaijinQuestMap.drawlayer or WorldMapButton.routes):CreateTexture(nil, "OVERLAY")
      path[nline]:SetWidth(4)
      path[nline]:SetHeight(4)
      path[nline]:SetTexture(RaijinQuestConfig.path.."\\img\\route")
      ...
      if minimap then -- draw minimap
        path[nline]:SetPoint("CENTER", RaijinQuestMap.drawlayer, "CENTER", -xpos, ypos)
      else -- draw worldmap
...
RaijinQuest.route = CreateFrame("Frame", "RaijinQuestRoute", WorldFrame)
RaijinQuest.route.firstnode = nil
RaijinQuest.route.coords = {}

RaijinQuest.route.Reset = function(self)
  self.coords = {}
  self.firstnode = nil
end

RaijinQuest.route.AddPoint = function(self, tbl)
  table.insert(self.coords, tbl)
  self.firstnode = nil
end

local targetTitle, targetCluster, targetLayer, targetTexture = nil, nil, nil, nil
RaijinQuest.route.SetTarget = function(node, default)
  if node and ( node.title ~= targetTitle
    or node.cluster ~= targetCluster
    or node.layer ~= targetLayer
    or node.texture ~= targetTexture )
  then
    RaijinQuestMap.queue_update = true
  end
...
RaijinQuest.route.IsTarget = function(node)
  if node then
...
RaijinQuest.route:SetScript("OnUpdate", function()
  ...
  if not this.recalculate or this.recalculate < GetTime() then
    ...
    if targetTitle and this.coords[1] and not RaijinQuest.route.IsTarget(this.coords[1][3]) then
      local target = nil

      -- check for the old index of the target
      for id, data in pairs(this.coords) do
        if RaijinQuest.route.IsTarget(data[3]) then
          target = id
          break
        end
      end
  ...
  if not wrongmap and this.coords[1] and this.coords[1][4] and not this.arrow:IsShown() and RaijinQuest_config["arrow"] == "1" and GetTime() > completed + 1 then
    this.arrow:Show()
  end

  -- abort without any nodes or distances
  if not this.coords[1] or not this.coords[1][4] or RaijinQuest_config["routes"] == "0" then
  ...
  if wrongmap then
    ...
    if RaijinQuest_config["routeminimap"] == "1" then
      DrawLine(mplayerpath,xplayer*100,yplayer*100,this.coords[1][1],this.coords[1][2],true,true)
    end
  end
end)

RaijinQuest.route.drawlayer = CreateFrame("Frame", "RaijinQuestRouteDrawLayer", WorldMapButton)
RaijinQuest.route.drawlayer:SetFrameLevel(113)
RaijinQuest.route.drawlayer:SetAllPoints()

WorldMapButton.routes = CreateFrame("Frame", "RaijinQuestRouteDisplay", RaijinQuest.route.drawlayer)
WorldMapButton.routes:SetAllPoints()

RaijinQuest.route.arrow = CreateFrame("Frame", "RaijinQuestRouteArrow", UIParent)
RaijinQuest.route.arrow:SetPoint("CENTER", 0, -100)
RaijinQuest.route.arrow:SetWidth(48)
RaijinQuest.route.arrow:SetHeight(36)
RaijinQuest.route.arrow:SetClampedToScreen(true)
RaijinQuest.route.arrow:SetMovable(true)
RaijinQuest.route.arrow:EnableMouse(true)
RaijinQuest.route.arrow:RegisterForDrag('LeftButton')
RaijinQuest.route.arrow:SetScript("OnDragStart", function()
  if IsShiftKeyDown() then
    this:StartMoving()
  end
end)

RaijinQuest.route.arrow:SetScript("OnDragStop", function()
...
RaijinQuest.route.arrow:SetScript("OnUpdate", function()
  ...
  if not target or wrongmap or RaijinQuest_config["arrow"] == "0" then
  ...
  player = RaijinQuestCompat.GetPlayerFacing()
  angle = angle - player
  ...
  if target ~= lasttarget then
    ...
    if tonumber(target[3]["qlvl"]) then
      color = RaijinQuestMap:HexDifficultyColor(tonumber(target[3]["qlvl"]))
    end
    ...
    if target[3].texture then
      ...
      this.texture:SetTexture(RaijinQuestConfig.path.."\\img\\node")
      this.texture:SetVertexColor(RaijinQuestMap.str2rgb(target[3].title))
    end
  ...
  if distance ~= this.distance.number then
    this.distance:SetText("|cffaaaaaa" .. RaijinQuest_Loc["Distance"] .. ": "..string.format("%.1f", distance))
...
RaijinQuest.route.arrow.texture = RaijinQuest.route.arrow:CreateTexture("RaijinQuestRouteNodeTexture", "OVERLAY")
RaijinQuest.route.arrow.texture:SetWidth(28)
RaijinQuest.route.arrow.texture:SetHeight(28)
RaijinQuest.route.arrow.texture:SetPoint("BOTTOM", 0, 0)

RaijinQuest.route.arrow.model = RaijinQuest.route.arrow:CreateTexture("RaijinQuestRouteArrow", "MEDIUM")
RaijinQuest.route.arrow.model:SetTexture(RaijinQuestConfig.path.."\\img\\arrow")
RaijinQuest.route.arrow.model:SetTexCoord(0,0,0.109375,0.08203125)
RaijinQuest.route.arrow.model:SetAllPoints()

RaijinQuest.route.arrow.title = RaijinQuest.route.arrow:CreateFontString("RaijinQuestRouteText", "HIGH", "GameFontWhite")
RaijinQuest.route.arrow.title:SetPoint("TOP", RaijinQuest.route.arrow.model, "BOTTOM", 0, -10)
RaijinQuest.route.arrow.title:SetFont(pfUI.font_default, pfUI_config.global.font_size+1, "OUTLINE")
RaijinQuest.route.arrow.title:SetTextColor(1,.8,0)
RaijinQuest.route.arrow.title:SetJustifyH("CENTER")

RaijinQuest.route.arrow.description = RaijinQuest.route.arrow:CreateFontString("RaijinQuestRouteText", "HIGH", "GameFontWhite")
RaijinQuest.route.arrow.description:SetPoint("TOP", RaijinQuest.route.arrow.title, "BOTTOM", 0, -2)
RaijinQuest.route.arrow.description:SetFont(pfUI.font_default, pfUI_config.global.font_size, "OUTLINE")
RaijinQuest.route.arrow.description:SetTextColor(1,1,1)
RaijinQuest.route.arrow.description:SetJustifyH("CENTER")

RaijinQuest.route.arrow.distance = RaijinQuest.route.arrow:CreateFontString("RaijinQuestRouteDistance", "HIGH", "GameFontWhite")
RaijinQuest.route.arrow.distance:SetPoint("TOP", RaijinQuest.route.arrow.description, "BOTTOM", 0, -2)
RaijinQuest.route.arrow.distance:SetFont(pfUI.font_default, pfUI_config.global.font_size-1, "OUTLINE")
RaijinQuest.route.arrow.distance:SetTextColor(.8,.8,.8)
RaijinQuest.route.arrow.distance:SetJustifyH("CENTER")

RaijinQuest.route.arrow.parent = RaijinQuest.route
