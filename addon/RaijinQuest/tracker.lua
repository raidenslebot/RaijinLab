...
local function ShowTooltip()
  if this.tooltip then
    ...
    if this.node and this.node.questid then
      if RaijinQuestDB["quests"] and RaijinQuestDB["quests"]["loc"] and RaijinQuestDB["quests"]["loc"][this.node.questid] and RaijinQuestDB["quests"]["loc"][this.node.questid]["O"] then
        GameTooltip:AddLine(RaijinQuestDatabase:FormatQuestText(RaijinQuestDB["quests"]["loc"][this.node.questid]["O"]), 1,1,1,1)
        GameTooltip:AddLine(" ")
      end

      local qlogid = RaijinQuest.questlog[this.node.questid] and RaijinQuest.questlog[this.node.questid].qlogid
      if qlogid then
        ...
        if objectives and objectives > 0 then
          for i=1, objectives, 1 do
            ...
            elseif cur and req then
              local r,g,b = RaijinQuestMap.tooltip:GetColor(cur, req)
...
tracker = CreateFrame("Frame", "RaijinQuestMapTracker", UIParent)
tracker:Hide()
...
tracker:SetScript("OnEvent", function()
  ...
  -- Defaulted: config.lua used to throw before its defaults were applied
  -- (vendored path detection), leaving every key nil - and a nil here is
  -- arithmetic on the next line.
  fontsize = tonumber(RaijinQuest_config["trackerfontsize"]) or 11
  entryheight = ceil(fontsize*1.6)

  -- restore tracker state
  if RaijinQuest_config["showtracker"] and RaijinQuest_config["showtracker"] == "0" then
    this:Hide()
  else
    this:Show()
  end
...
tracker:SetScript("OnMouseDown",function()
  if not RaijinQuest_config.lock then
    this:StartMoving()
  end
end)
...
tracker:SetScript("OnMouseUp",function()
  ...
  -- save position
  RaijinQuest_config.trackerpos = { anchor, x, y }
end)
...
tracker:SetScript("OnUpdate", function()
  ...
  if RaijinQuestCompat.QuestWatchFrame:IsShown() then
    RaijinQuestCompat.QuestWatchFrame:Hide()
  end
end)

tracker:SetScript("OnShow", function()
  RaijinQuest_config["showtracker"] = "1"

  -- load tracker position if exists
   if RaijinQuest_config.trackerpos then
     this:ClearAllPoints()
     this:SetPoint(unpack(RaijinQuest_config.trackerpos))
   end
end)

tracker:SetScript("OnHide", function()
  RaijinQuest_config["showtracker"] = "0"
end)
...
do -- button panel
  ...
  local function CreateButton(icon, anchor, tooltip, func)
    ...
    b.icon:SetAllPoints()
    b.icon:SetTexture(RaijinQuestConfig.path.."\\img\\tracker_"..icon)
  ...
  tracker.btnquest = CreateButton("quests", "TOPLEFT", RaijinQuest_Loc["Show Current Quests"], function()
    tracker.mode = "QUEST_TRACKING"
    RaijinQuestMap:UpdateNodes()
  end)

  tracker.btndatabase = CreateButton("database", "TOPLEFT", RaijinQuest_Loc["Show Database Results"], function()
    tracker.mode = "DATABASE_TRACKING"
    RaijinQuestMap:UpdateNodes()
  end)

  tracker.btngiver = CreateButton("giver", "TOPLEFT", RaijinQuest_Loc["Show Quest Givers"], function()
    tracker.mode = "GIVER_TRACKING"
    RaijinQuestMap:UpdateNodes()
  end)

  tracker.btnclose = CreateButton("close", "TOPRIGHT", RaijinQuest_Loc["Close Tracker"], function()
    DEFAULT_CHAT_FRAME:AddMessage(RaijinQuest_Loc["|cff33ffccpf|cffffffffQuest: Tracker is now hidden. Type `/db tracker` to show."])
    tracker:Hide()
  end)

  tracker.btnsettings = CreateButton("settings", "TOPRIGHT", RaijinQuest_Loc["Open Settings"], function()
    if RaijinQuestConfig then RaijinQuestConfig:Show() end
  end)

  tracker.btnclean = CreateButton("clean", "TOPRIGHT", RaijinQuest_Loc["Clean Database Results"], function()
    RaijinQuestMap:DeleteNode("PFDB")
    RaijinQuestMap:UpdateNodes()
  end)

  tracker.btnsearch = CreateButton("search", "TOPRIGHT", RaijinQuest_Loc["Open Database Browser"], function()
    if RaijinQuestBrowser then RaijinQuestBrowser:Show() end
  end)
end

function tracker.ButtonEnter()
  RaijinQuestMap.highlight = this.title
  ShowTooltip()
end
...
function tracker.ButtonUpdate()
  local alpha = tonumber((RaijinQuest_config["trackeralpha"] or .2)) or .2
  ...
  if RaijinQuestMap.highlight and RaijinQuestMap.highlight == this.title then
...
function tracker.ButtonClick()
  ...
  elseif IsShiftKeyDown() then
    ...
    if this.node.questid and not this.node.qlogid then
      ...
      RaijinQuest_history[this.node.questid] = { time(), UnitLevel("player") }
    ...
    RaijinQuestMap:DeleteNode(this.node.addon, this.title)
    RaijinQuestMap:UpdateNodes()
  ...
  elseif IsControlKeyDown() and RaijinQuest_config["spawncolors"] == "0" then
    -- switch color
    RaijinQuest_colors[this.title] = { RaijinQuestMap.str2rgb(this.title .. GetTime()) }
    RaijinQuestMap:UpdateNodes()
...
function tracker.ButtonEvent(self)
  ...
  elseif RaijinQuest_config["spawncolors"] == "1" then
    self.icon:SetTexture(RaijinQuestConfig.path.."\\img\\available_c")
    self.icon:SetVertexColor(1,1,1,1)
  else
    self.icon:SetTexture(RaijinQuestConfig.path.."\\img\\node")
    self.icon:SetVertexColor(RaijinQuestMap.str2rgb(title))
  end

  if tracker.mode == "QUEST_TRACKING" then
    local qlogid = RaijinQuest.questlog[qid] and RaijinQuest.questlog[qid].qlogid or 0
    ...
    local color = RaijinQuestCompat.GetDifficultyColor(level)
    local cur,max = 0,0
    local percent = 0

    -- write expand state
    if not expand_states[title] then
      expand_states[title] = RaijinQuest_config["trackerexpand"] == "1" and 1 or 0
    end
    ...
    local r,g,b = RaijinQuestMap.tooltip:GetColor(cur, max)
    local colorperc = string.format("|cff%02x%02x%02x", r*255, g*255, b*255)
    local showlevel = RaijinQuest_config["trackerlevel"] == "1" and "[" .. ( level or "??" ) .. ( tag and "+" or "") .. "] " or ""
  ...
  elseif tracker.mode == "GIVER_TRACKING" then
    ...
    local showlevel = RaijinQuest_config["trackerlevel"] == "1" and "[" .. ( level or "??" ) .. "] " or ""
    self.text:SetTextColor(color.r, color.g, color.b)
    self.text:SetText(showlevel .. title)
    self.level = tonumber(level)
    self.tooltip = RaijinQuest_Loc["|cff33ffcc<Ctrl-Click>|r Show Map / Toggle Color\n|cff33ffcc<Shift-Click>|r Mark As Done"]
  elseif tracker.mode == "DATABASE_TRACKING" then
    self.text:SetText(title)
    self.text:SetTextColor(1,1,1,1)
    self.text:SetTextColor(RaijinQuestMap.str2rgb(title))
    self.tooltip = RaijinQuest_Loc["|cff33ffcc<Ctrl-Click>|r Show Map / Toggle Color\n|cff33ffcc<Shift-Click>|r Hide Nodes"]
  end
...
function tracker.ButtonAdd(title, node)
  ...
  local questid = title
  for qid, data in pairs(RaijinQuest.questlog) do
  ...
  if tracker.mode == "QUEST_TRACKING" then -- skip everything that isn't in questlog
    ...
    if not RaijinQuest.questlog or not RaijinQuest.questlog[questid] then return end
  elseif tracker.mode == "GIVER_TRACKING" then -- skip everything that isn't a questgiver
    if node.addon ~= "PFQUEST" then return end
    -- break on already taken quests
    if not RaijinQuest.questlog or RaijinQuest.questlog[questid] then return end
  ...
  if not tracker.buttons[id] then
    tracker.buttons[id] = CreateFrame("Button", "RaijinQuestMapButton"..id, tracker)
    ...
    tracker.buttons[id].text = tracker.buttons[id]:CreateFontString("RaijinQuestIDButton", "HIGH", "GameFontNormal")
...
function tracker.Reset()
  ...
  for qlogid=1,40 do
    ...
    if title and not header then
      local watched = IsQuestWatched(qlogid)
      if watched then
        local img = complete and RaijinQuestConfig.path.."\\img\\complete_c" or RaijinQuestConfig.path.."\\img\\complete"
        RaijinQuest.tracker.ButtonAdd(title, { dummy = true, addon = "PFQUEST", texture = img })
      end
