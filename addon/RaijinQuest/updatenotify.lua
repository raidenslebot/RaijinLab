...
versioncheck:SetScript("OnEvent", function()
  if event == "ADDON_LOADED" then
    ...
    return
  elseif event == "CHAT_MSG_ADDON" and arg1 == "RaijinQuest" then
    ...
    if v == "VERSION" and remoteversion then
      ...
      if remote > version then RaijinQuest_config.latest = remote end
    end
    return
  ...
  -- send updates
  for _, chan in pairs(channels) do SendAddonMessage("RaijinQuest", "VERSION:" .. version, chan) end

  -- abort here on group member events
  if event == "PARTY_MEMBERS_CHANGED" then return end

  -- display available update
  if version and version > 0 and RaijinQuest_config.latest and RaijinQuest_config.latest > version and not displayed then
    DEFAULT_CHAT_FRAME:AddMessage(RaijinQuest_Loc["|cff33ffccpf|rQuest: New version available! Have a look at http://shagu.org !"])
    displayed = true
  end
end)
