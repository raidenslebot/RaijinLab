...
void ArmUnlock() {
    ...
    if (!g_armed) {
        g_armed = true;
        RL::Log::Warn("Actions: armed hwflag=0x%08X taint=0x%08X",
                      (unsigned)Addr::HardwareEventFlag, (unsigned)Addr::TaintContext);
    }
}
...
// from the cast wire (mid-combat nested pcall crash).
